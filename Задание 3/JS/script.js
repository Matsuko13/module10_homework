...
const sendGeoMessageButton = document.querySelector(".geo__lokatsiya");
...
function initEchoWebSocket() {
    echoWebSocket = new WebSocket("wss://echo.websocket.org/");

    echoWebSocket.onopen = () => {
        addMessage(getServerMessageCode("Соединение успешно установлено."));
...
function sendGeoButtonClickHandler() {
    if ("geolocation" in navigator) {
        ...
        function success(position) {
            const link = `https://www.openstreetmap.org/ ${position.coords.latitude}/${position.coords.longitude}`;
